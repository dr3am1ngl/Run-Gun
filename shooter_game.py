...
class player(GameSprite):
    def update(self):
        ...
        if keys_pressed[K_LEFT] and self.rect_x > 5:
            self.rect.x -= self.speed
        if keys_pressed[K_RIGHT] and self.rect_x < 420:
            self.rect.x +=self.speed
    def fire(self):
...
class Enemy(GameSprite):
    def update(self):
        ...
        global lost
        if self.rect_y > 500:  
                self.rect_y = randit = randit(80, 420)
                self.rect_y = 0
                lost = lost = 1 

        
hero = player('rocket.png',250,400,5)
bullets = sprite.Group()

game = True


enemy = Enemy('rocket.png',250,400,5)
game = True
FPS = 120
...
for c in collides:
    score += 1
    monsters = Enemy("ufo.png", randint(80,620),0,randit(1,5))
...
win = font1.render('ю вин бот', True, (255, 255, 255))
...
max_lost = 1000 #проиграли, если пропустили столько
